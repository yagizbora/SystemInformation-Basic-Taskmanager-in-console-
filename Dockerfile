...
WORKDIR /src
COPY ["SystemInformation(Basic Taskmanager in console).csproj", "."]
RUN dotnet restore "./SystemInformation(Basic Taskmanager in console).csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./SystemInformation(Basic Taskmanager in console).csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Bu aşama, son aşamaya kopyalanacak hizmet projesini yayımlamak için kullanılır
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./SystemInformation(Basic Taskmanager in console).csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "SystemInformation(Basic Taskmanager in console).dll"]